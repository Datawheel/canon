...
const spanify = s => s.length ? s.replace(/([A-z0-9]*\{\{[^\}]+\}\})/g, "<span class=\"notranslate\">$1</span>") : s;
const varify = s => s.length ? s.replace(/\<span class\=\"notranslate\"\>([A-z0-9]*\{\{[^\}]+\}\})\<\/span\>/g, "$1") : s;
