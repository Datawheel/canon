...
const migrate = async() => {
  ...
  const dbold = await fetchOldModel("/db_0.14", false);
  const dbnew = await fetchNewModel("/db_0.17", true);
  ...
  console.log(`Successfully migrated from CMS version 0.14/0.15/0.16 on ${oldDBName} to CMS version 0.17 on ${newDBName}`);
  shell.exit(0);
};
