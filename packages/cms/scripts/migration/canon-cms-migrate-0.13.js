...
const migrate = async() => {
  ...
  const dbold = await fetchOldModel("/db_0.13", false);
  const dbnew = await fetchNewModel("/db_0.14", true);
  ...
  console.log(`Successfully migrated from CMS version 0.13 on ${oldDBName} to CMS version 0.14 on ${newDBName}`);
  shell.exit(0);
};
