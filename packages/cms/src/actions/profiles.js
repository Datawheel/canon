...
export function fetchVariables(config) {
  return async function(dispatch, getStore) {
    ...
    const thisProfile = isStory
      ? getStore().cms.stories.find(p => p.id === currentStoryPid)
      : getStore().cms.profiles.find(p => p.id === currentPid);
    ...
    */
    ...
    for (const thisLocale of locales) {
      // Stories don't have customAttributes
      if (isStory) {
        attributesByLocale[thisLocale] = {};
    ...
    const thisID = isStory ? currentStoryPid : currentPid;
    ...
    for (const thisLocale of locales) {
      ...
      if (config.type.includes("generator")) {
        let gids = [];
        // If given an id, clear only that generator.
        if (config.id) {
          gids = [config.id];
        // otherwise, this is a fresh run - clear the entire variables object
        }
        else {
