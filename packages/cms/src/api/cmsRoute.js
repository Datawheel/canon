...
module.exports = function(app) {
  ...
  app.get("/api/cms/formattertree", async(req, res) => {
    const formatters = await db.formatter.findAll(formatterReqTreeOnly);
  ...
  getList.forEach(ref => {
    app.get(`/api/cms/${ref}/get/:id`, async(req, res) => {
      if (contentTables.includes(ref)) {
        const u = await db[ref].findOne({where: {id: req.params.id}, include: {association: "content"}});
        return res.json(u);
      }
      else {
        const u = await db[ref].findOne({where: {id: req.params.id}});
        return res.json(u);
      }
    });
  });
  ...
  newList.forEach(ref => {
    app.post(`/api/cms/${ref}/new`, isEnabled, async(req, res) => {
      ...
      const newObj = await db[ref].create(req.body);
      ...
      if (contentTables.includes(ref)) {
        ...
        await db[`${ref}_content`].create(payload);
        const fullObj = await db[ref].findOne({where: {id: newObj.id}, include: [{association: "content"}]});
  ...
  app.post("/api/cms/profile/newScaffold", isEnabled, async(req, res) => {
    ...
    const profile = await db.profile.create({slug: profileData.slug, ordering: profileData.ordering, dimension: profileData.dimName, levels: profileData.levels});
    await db.profile_content.create({id: profile.id, lang: envLoc});
    const topic = await db.topic.create({ordering: 0, profile_id: profile.id});
    await db.topic_content.create({id: topic.id, lang: envLoc});
    let profiles = await db.profile.findAll(profileReqTreeOnly);
    profiles = sortProfileTree(db, profiles);
  ...
  updateList.forEach(ref => {
    app.post(`/api/cms/${ref}/update`, isEnabled, async(req, res) => {
      const o = await db[ref].update(req.body, {where: {id: req.body.id}});
      if (contentTables.includes(ref) && req.body.content) {
        req.body.content.forEach(async content => {
          await db[`${ref}_content`].upsert(content, {where: {id: req.body.id, lang: content.lang}});
        });
      }
      return res.json(o);
    });
  ...
  deleteList.forEach(list => {
    list.elements.forEach(ref => {
      app.delete(`/api/cms/${ref}/delete`, isEnabled, async(req, res) => {
        const row = await db[ref].findOne({where: {id: req.query.id}});
        ...
        await db[ref].update({ordering: sequelize.literal("ordering -1")}, {where: where1});
        await db[ref].destroy({where: {id: req.query.id}});
        const where2 = {};
        where2[list.parent] = row[list.parent];
        const rows = await db[ref].findAll({where: where2, attributes: ["id", "ordering"], order: [["ordering", "ASC"]]});
  ...
  app.delete("/api/cms/generator/delete", isEnabled, async(req, res) => {
    const row = await db.generator.findOne({where: {id: req.query.id}});
    await db.generator.destroy({where: {id: req.query.id}});
    const rows = await db.generator.findAll({where: {profile_id: row.profile_id}, attributes: ["id", "name"]});
    return res.json(rows);
  });

  app.delete("/api/cms/materializer/delete", isEnabled, async(req, res) => {
    const row = await db.materializer.findOne({where: {id: req.query.id}});
    await db.materializer.update({ordering: sequelize.literal("ordering -1")}, {where: {profile_id: row.profile_id, ordering: {[Op.gt]: row.ordering}}});
    await db.materializer.destroy({where: {id: req.query.id}});
    const rows = await db.materializer.findAll({where: {profile_id: row.profile_id}, attributes: ["id", "ordering", "name"], order: [["ordering", "ASC"]]});
    return res.json(rows);
  });

  app.delete("/api/cms/profile/delete", isEnabled, async(req, res) => {
    const row = await db.profile.findOne({where: {id: req.query.id}});
    await db.profile.update({ordering: sequelize.literal("ordering -1")}, {where: {ordering: {[Op.gt]: row.ordering}}});
    await db.profile.destroy({where: {id: req.query.id}});
  ...
  app.delete("/api/cms/story/delete", isEnabled, async(req, res) => {
    const row = await db.story.findOne({where: {id: req.query.id}});
    await db.story.update({ordering: sequelize.literal("ordering -1")}, {where: {ordering: {[Op.gt]: row.ordering}}});
    await db.story.destroy({where: {id: req.query.id}});
    let stories = await db.story.findAll(storyReqTreeOnly);
    stories = sortStoryTree(db, stories);
    return res.json(stories);
  });
  ...
  app.delete("/api/cms/formatter/delete", isEnabled, async(req, res) => {
    await db.formatter.destroy({where: {id: req.query.id}});
    const rows = await db.formatter.findAll({attributes: ["id", "name", "description"]});
    return res.json(rows);
  });

  app.delete("/api/cms/topic/delete", isEnabled, async(req, res) => {
    const row = await db.topic.findOne({where: {id: req.query.id}});
    await db.topic.update({ordering: sequelize.literal("ordering -1")}, {where: {profile_id: row.profile_id, ordering: {[Op.gt]: row.ordering}}});
    await db.topic.destroy({where: {id: req.query.id}});
  ...
  app.delete("/api/cms/storytopic/delete", isEnabled, async(req, res) => {
    const row = await db.storytopic.findOne({where: {id: req.query.id}});
    await db.storytopic.update({ordering: sequelize.literal("ordering -1")}, {where: {story_id: row.story_id, ordering: {[Op.gt]: row.ordering}}});
    await db.storytopic.destroy({where: {id: req.query.id}});
  ...
  app.delete("/api/cms/selector/delete", isEnabled, async(req, res) => {
    const row = await db.selector.findOne({where: {id: req.query.id}});
    await db.selector.update({ordering: sequelize.literal("ordering -1")}, {where: {topic_id: row.topic_id, ordering: {[Op.gt]: row.ordering}}});
    await db.selector.destroy({where: {id: req.query.id}});
    const rows = await db.selector.findAll({where: {topic_id: row.topic_id}, order: [["ordering", "ASC"]]});
    return res.json(rows);
  });
