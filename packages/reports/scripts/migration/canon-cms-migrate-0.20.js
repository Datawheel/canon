...
const migrate = async() => {
  ...
  const dbold = await fetchOldModel("/db_0.17", false);
  const dbnew = await fetchNewModel("/db_0.21", true);
  ...
  console.log(`Successfully migrated from CMS version 0.17/0.18/0.19/0.20 on ${oldDBName} to CMS version 0.21 on ${newDBName}`);
  shell.exit(0);
};
